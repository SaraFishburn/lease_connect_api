class SessionsController < ApplicationController
  def sign_in
    ...
    if user
      ...
      render json: { user: user.to_json(except: %i[id password_digest]) }
    else
      render json: { error: 'Invalid email or password' }, status: :unprocessable_entity
    end
  end

  # method to delete login token and sign user out
  def sign_out
    # check if there is a current user token and delete if true
    if current_user
      cookies.delete('login')
    else
      render json: { error: 'Must be signed in to sign out' }, status: :unprocessable_entity
    end
  end
end
